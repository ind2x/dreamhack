...
하하... 나는 ```flag``` 유저의 패스워드가 플래그일 줄 알고 그걸 가져오려 했는데, 값을 변경하면 result 값이 뜨면서 암호문처럼 생긴 값이 나타난다.

근데 자세히 보면.. 어떤 값으로 comment를 변경하든, **나타나는 메시지 값이 동일하다!!**

이걸 base64 decode를 했더니.. 플래그가 나왔다...ㅠ
